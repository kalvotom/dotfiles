...
beautiful.init("/home/kalvin/.config/awesome/themes/zenburn/theme.lua")
...
myoffice = {
  ...
  { "mathematica", "Mathematica" },
	{ "ijulia", "ipython notebook --profile julia" },
  { "PDF XChange", "pdfxchange"},
  { "XMind", "XMind" },
  { "LyX", "lyx" }
}
...
mytextclock:buttons(awful.util.table.join(
   awful.button({ }, 1, function ()awful.util.spawn("gsimplecal", false) end)
           ))
-----Vicious by Giulio Bortot
...
-- MY Vol Icon --by umberto reale
vol_menu = awful.menu({ items = {
          { "Mute", "amixer -q set Master mute"},
          { "Increase", "amixer -q set Master 2+ unmute"},
          { "Decrease", "amixer -q set Master 2- unmute"},
          { "Mixer", terminal .. " -e alsamixer"}
        }
      })
---cpufrequencymenu---
cpu_menu = awful.menu({ items = { { "Powersave", "sudo cpupower frequency-set -g powersave"},
...
myvoltext:set_text('Vol: ')
myvoltext:buttons(awful.util.table.join(
   awful.button({ }, 3, function () vol_menu:toggle() end )
         ))

-- Vol bar Widget ---by Giulio Bortot
...
--volbar:set_color("#FFFFFF") --beautiful.fg_normal)
...
baticon:set_text('Batt: ')
baticon:buttons(awful.util.table.join(
   awful.button({ }, 3, function () cpu_menu:toggle() end )
           ))
-----------------
--- Batt widget ---
...
awful.rules.rules = {
    ...
    { rule = { class = "URxvt" }, properties = { opacity = 0.8 }},
		{ rule = { class = "XTerm" }, properties = { opacity = 0.7 }},
    { rule = { class = "Pidgin" }, properties = { opacity = 0.95, floating = true }},
...
--  run_once("compton -cCG -fF -o 0.9 -r 10 -m 0.8")
--  run_once("compton -cCGfF -o 0.38 -O 20 -I 20 -t 0.02 -l 0.02 -r 5.0 -D 200")
--  run_once("google-musicmanager")
  run_once("compton -cCG -o 0.8 -r 5 -l 0.3 -t 0.3 -D 200 -r 6")
  run_once("nm-applet")
  run_once("mpd ~/.mpdconf")
--  run_once("cbatticon")
--      run_once("mpdnotify start")
